...
tab:AddSwitch('Auto Click', function(a)
    auto_click = a
end)
local auto_use_atc = false
tab:AddSwitch('Auto Use Moves)', function(a)
    auto_use_atc = a
end)
local whitelist = ''
local drop = tab:AddDropdown('Choose Mob', function(a)
...
spawn(function()
    while wait() do
        for i,v in pairs(game:GetService("Workspace").NPCSpawns:GetChildren()) do 
            if v:IsA("MeshPart") and v:FindFirstChildWhichIsA("Model") and not string.find(v.Name, "Human") and not string.find(v.Name, "Boss") and string.find(v:FindFirstChildWhichIsA("Model").Name, whitelist) then
...
local tab_focus = w:AddTab('Focus Settings')
local psy = false
local kag = false
local dur = false
local speeed = false
